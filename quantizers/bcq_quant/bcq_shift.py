...
def quantize_shift(w, qbits, rounds=15, group_size=-1, transpose=False, use_bst=True, apot_nums = 3, use_tcq=False):
    '''
    ...
    ret, B, alpha = greedy_mean_torch(w_, n_bits=qbits, wf=wf, use_tcq=use_tcq)
    if rounds > 0 and qbits > 1:
        for _ in range(rounds):
            ret, B, alpha = refine_mean_torch(w_, ret, B, alpha, wf=wf, use_bst=use_bst, apot_nums=apot_nums, use_tcq=use_tcq)
...
def greedy_mean_torch(w, n_bits=1, wf=None, q_bias=None, scale=None, shift = False, use_tcq=False):
    ...
    for i in range(n_bits):
        if use_tcq:
            threshold = torch.quantile(torch.abs(r), 1/3)
            mask = torch.abs(r) < threshold
            b = r.sign()
            b[mask] = 0
        else:
            b = r.sign()
...
def refine_mean_torch(w, w_hat, B, Alpha, wf=None, use_bst=True, apot_nums=1, use_tcq=False):
    w = w.float()
    d1, d2 = w.shape
    with torch.no_grad():
        n_bits = B.shape[-1]
        ...
        if use_bst == False:
            ...
            if use_tcq:
                B_new = find_ternary_B_torch(w, Alpha_new)
            else:
...
def list_ternary_vecs(n):
    ListTernaryVecs = {0 : [[]]}
    for m in range(1, n+1):
        ListTernaryVecs[m] = [[1.] + l for l in ListTernaryVecs[m-1]] + [[-1.] + l for l in ListTernaryVecs[m-1]] + [[0] + l for l in ListTernaryVecs[m-1]]
    return ListTernaryVecs

def find_ternary_B_torch(w, Alpha):
    '''Find optimal quantization assignment via binary search (torch)'''
    n_bits = Alpha.shape[-1]

    ListTernaryVecs = list_ternary_vecs(n_bits)
    ten_mat = torch.from_numpy(np.vstack(ListTernaryVecs[n_bits]).astype(np.float32)).to(w.device)

    d1, d2 = w.shape
    row_inds = torch.arange(d1, dtype=torch.long).view(d1, 1).repeat([1, d2]).view(-1)
    # w is d1xd2, Alpha is d1xk, v is d1x2^k
    v = Alpha.mm(ten_mat.t())
    v_sorted, inds = torch.sort(v)
    # Binary search to find nearest neighbor
    w_flat = w.view([-1])
    Left = torch.zeros(d1*d2, dtype=torch.int, device=w.device)
    Right = torch.ones(d1*d2, dtype=torch.int, device=w.device) * (3 ** n_bits - 1)
    for i in range(int(np.ceil(np.log2(3 ** n_bits)))):
    ...
    return ten_mat[assignment_inds, :]
